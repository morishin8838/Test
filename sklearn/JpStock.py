...
class JpStock:
    ...
    def get(self, code, start=None, end=None, interval='d'):
        if code in {'N225', 'GSPC', 'IXIC', 'DJI'}:
            ...
            result = data.DataReader("".join(['^', code]),'yahoo', start, end)
        ...
        result.columns = ['Date', 'Open', 'High', 'Low', 'Close', 'Volume', 'AdjClose']
    ...
    if sys.version_info > (version):
        if len(sys.argv) > argsmin:
            try:
                ...
                jpstock = JpStock()
                stock_tse = jpstock.get(stock, start=start,end=end)
                print("Complete :" +stock)
                stock_tse.to_csv("".join(["stock_", stock, ".csv"]),sep=",", index_label="Date")
